...
- **User Profile** `[hl_user_profile]` - Unified user profile page replacing BuddyBoss profiles. Route: `?user_id=X` or `?enrollment_id=X` (no param = own profile). Gradient hero with avatar, name, role badges, school, circular completion indicator. Overview tab with 3-4 info cards (Contact, Program, Team & Coaching, Classrooms). Cycle selector pill bar for multi-enrollment users. Role-based access control (own profile, admin/staff, coach→assigned mentors, mentor→team members, school leader→school staff). BB profile redirect via `template_redirect` (escape hatch: `?bb=1`). Placeholder tabs for Progress, Coaching, Assessments, RP & Observations, Manage (admin). Responsive layout with mobile tab icon-only mode.
