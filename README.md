...
**Status:** v1 complete — Phases 1-21 done (26 shortcode pages, 15 admin pages, 35 DB tables, tabbed cohort editor)
...
- [x] **21.5 Fix Cohort Group Save Bug** — Investigation found code path correct end-to-end (domain model, repository, admin controller, installer). Added explicit `$format` array to `$wpdb->update()` in cohort repository for type-safety. Added `error_log` diagnostics for failed updates and post-save mismatch detection. Schema revision 8 ensures migration runs on staging.
- [x] **21.6 Frontend — Wire Assessment Activities in Program Page** — Program Page: batch-load assessment instance statuses per enrollment, status-aware button labels for teacher/children assessments (Start/Continue/View Submitted), drip date badge (`hl-drip-badge`) for time-gated locked activities, children assessment buttons route directly to assessment page with `activity_id` param. Activity Page: pass `activity_id` to children assessment URL for instance resolution. CSS: amber drip badge pill.
