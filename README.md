...
**Status:** v1 complete — Phases 1-20 done, Phase 21 in progress (26 shortcode pages, 15 admin pages, 35 DB tables, tabbed cohort editor)
...
- [~] **21.5 Fix Cohort Group Save Bug** — DONE: Added explicit `$format` array to `$wpdb->update()` in cohort repository, self-healing `ensure_cohort_group_column()` in admin save handler (detects missing column → adds it → retries update), hardened installer migration with retry logic and post-add verification, comprehensive error_log diagnostics. TODO: Test on staging to confirm the fix works. Most likely root cause is the `cohort_group_id` column missing from the DB table on staging (dbDelta/ALTER TABLE failed silently during earlier deployment).
- [~] **21.6 Frontend — Wire Assessment Activities in Program Page** — DONE: Program Page batch-loads assessment instance statuses per enrollment, status-aware button labels for teacher/children assessments (Start/Continue/View Submitted), drip date badge (`hl-drip-badge`) for time-gated locked activities, children assessment buttons route directly to assessment page with `activity_id` param. Activity Page passes `activity_id` to children assessment URL for instance resolution. CSS: amber drip badge pill. TODO: Test on staging to verify routing and button labels render correctly.
