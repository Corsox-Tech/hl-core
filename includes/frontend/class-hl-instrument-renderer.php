...
class HL_Instrument_Renderer {
    ...
    /** @var array Full instance data (display_name, school_name, classroom_name, phase, cycle_name, etc.). */
    private $instance;
     ...
     */
    public static function create_multi_age_group( $classroom_id, $cycle_id, $instance_id, $existing_answers = array(), $instance = array(), $children = null ) {
        global $wpdb;
        ...
        $snapshots = HL_Child_Snapshot_Service::get_snapshots_for_classroom( $classroom_id, $cycle_id );

        // 3. Ensure snapshots exist for any children missing them.
        foreach ( $children as $child ) {
            $child = (object) $child;
            if ( ! isset( $snapshots[ $child->child_id ] ) && ! empty( $child->dob ) ) {
                HL_Child_Snapshot_Service::ensure_snapshot( $child->child_id, $cycle_id, $child->dob );
    ...
    private function render_inline_styles() {
        ?>
        <style>
            /* Static CSS is in frontend.css — only dynamic overrides here */
