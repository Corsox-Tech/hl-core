...
class HL_Coaching_Service {
    ...
    /**
     * Get coaching sessions by partnership
     */
    public function get_by_partnership($partnership_id) {
        ...
        return $wpdb->get_results($wpdb->prepare(
            "SELECT cs.*, u_coach.display_name as coach_name, u_mentor.display_name as mentor_name
             ...
             WHERE cs.partnership_id = %d ORDER BY cs.session_datetime DESC, cs.created_at DESC",
            $partnership_id
        ), ARRAY_A) ?: array();
    }
     ...
     * @return array
     */
    public function get_sessions_for_participant($enrollment_id, $partnership_id) {
        global $wpdb;
     ...
     * @return array
     */
    public function get_upcoming_sessions($enrollment_id, $partnership_id) {
        global $wpdb;
     ...
     * @return array
     */
    public function get_past_sessions($enrollment_id, $partnership_id) {
        global $wpdb;
        $now = current_time('mysql');
    ...
    /**
     * Check if cancellation is allowed for a partnership.
     *
     * @param int $partnership_id
     * @return bool
     */
    public function is_cancellation_allowed($partnership_id) {
        global $wpdb;

        $settings_json = $wpdb->get_var($wpdb->prepare(
            "SELECT settings FROM {$wpdb->prefix}hl_partnership WHERE partnership_id = %d",
            $partnership_id
        ));
    ...
    public function mark_attendance($session_id, $status) {
        ...
        if ($status === 'attended') {
            $session = $wpdb->get_row($wpdb->prepare(
                "SELECT partnership_id, mentor_enrollment_id FROM {$wpdb->prefix}hl_coaching_session WHERE session_id = %d",
                $session_id
            ));
    ...
    private function update_coaching_component_state($enrollment_id, $partnership_id) {
        ...
        $attended_count = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}hl_coaching_session
             WHERE mentor_enrollment_id = %d AND partnership_id = %d
     ...
     *
     * @param array $data Keys: partnership_id, coach_user_id, mentor_enrollment_id,
    ...
    public function create_session($data) {
        ...
        if (empty($data['partnership_id'])) {
            return new WP_Error('missing_partnership', __('Partnership is required.', 'hl-core'));
        }
        ...
        HL_Audit_Service::log('coaching_session.created', array(
            'entity_type' => 'coaching_session',
            'entity_id'   => $session_id,
            'partnership_id'   => $insert_data['partnership_id'],
            'after_data'  => array(
                'partnership_id'            => $insert_data['partnership_id'],
     ...
     * @return array
     */
    public function get_available_observations($session_id, $partnership_id, $mentor_enrollment_id) {
        global $wpdb;
        ...
        return $wpdb->get_results($wpdb->prepare(
            ...
            $partnership_id, $mentor_enrollment_id, $session_id
        ), ARRAY_A) ?: array();
    }
