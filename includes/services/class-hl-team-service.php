...
class HL_Team_Service {
    ...
    public function create_team($data) {
        if (empty($data['team_name']) || empty($data['cycle_id']) || empty($data['school_id'])) {
            return new WP_Error('missing_fields', __('Team name, cycle, and school are required.', 'hl-core'));
        }
     ...
     *
     * Enforces:
     * - Hard constraint: 1 team per enrollment per cycle
    ...
    public function add_member($team_id, $enrollment_id, $membership_type = 'member', $force_override = false) {
        ...
        if ($existing_team_id) {
            ...
            return new WP_Error('one_team_per_cycle', __('This participant is already assigned to another team in this cycle. Remove them from the other team first.', 'hl-core'));
        }
