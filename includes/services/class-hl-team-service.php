...
class HL_Team_Service {
    ...
    public function create_team($data) {
        if (empty($data['team_name']) || empty($data['partnership_id']) || empty($data['school_id'])) {
            return new WP_Error('missing_fields', __('Team name, partnership, and school are required.', 'hl-core'));
        }
     ...
     *
     * Enforces:
     * - Hard constraint: 1 team per enrollment per partnership
    ...
    public function add_member($team_id, $enrollment_id, $membership_type = 'member', $force_override = false) {
        ...
        $existing_team_id = $wpdb->get_var($wpdb->prepare(
            ...
            $enrollment_id, $team->partnership_id
        ));
        ...
        if ($existing_team_id) {
            ...
            return new WP_Error('one_team_per_partnership', __('This participant is already assigned to another team in this partnership. Remove them from the other team first.', 'hl-core'));
        }
