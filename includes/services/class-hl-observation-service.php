...
class HL_Observation_Service {
     ...
     *
     * Joins enrollments, users, classrooms, and cycles to provide
     * mentor_name, teacher_name, classroom_name, and cycle_name.
     *
    ...
    public function get_observation( $observation_id ) {
        ...
        $row = $wpdb->get_row( $wpdb->prepare(
            "SELECT o.*,
                    ...
                    mentor_e.cycle_id    AS mentor_cycle_id,
             ...
             LEFT JOIN {$wpdb->prefix}hl_cycle t ON o.cycle_id = t.cycle_id
    ...
    /**
     * Get observations by cycle with full joined data.
     *
     ...
     * teacher name, classroom name, and cycle name.
     *
    ...
    /**
     * Get all observations for a given mentor user across all cycles.
     *
     ...
     *
     * @param array $data Keys: cycle_id (required), mentor_enrollment_id (required),
    ...
    public function create_observation( $data ) {
        ...
        // Verify the mentor enrollment exists and belongs to the cycle
        ...
        if ( (int) $mentor_enrollment->cycle_id !== absint( $data['cycle_id'] ) ) {
            return new WP_Error( 'cycle_mismatch', __( 'Mentor enrollment does not belong to the specified cycle.', 'hl-core' ) );
        }
    ...
    /**
     * Get the classrooms a teacher is assigned to within a specific cycle.
     *
     ...
     * the enrollment is active, with cycle name included.
     *
     * @param int $user_id WordPress user ID.
     * @return array Array of enrollment rows (ARRAY_A) with cycle_name.
     */
    public function get_mentor_enrollments( $user_id ) {
        global $wpdb;

        $rows = $wpdb->get_results( $wpdb->prepare(
            "SELECT e.*, t.cycle_name
     ...
     */
    public function get_observation_component( $cycle_id ) {
        global $wpdb;
