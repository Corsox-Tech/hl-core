 ...
 * (who observed whom, in which classroom/partnership) while JetFormBuilder
...
class HL_Observation_Service {
     ...
     *
     * Joins enrollments, users, classrooms, and partnerships to provide
     * mentor_name, teacher_name, classroom_name, and partnership_name.
     *
    ...
    public function get_observation( $observation_id ) {
        ...
        $row = $wpdb->get_row( $wpdb->prepare(
            "SELECT o.*,
                    ...
                    mentor_e.partnership_id    AS mentor_partnership_id,
             ...
             LEFT JOIN {$wpdb->prefix}hl_partnership t ON o.partnership_id = t.partnership_id
    ...
    /**
     * Get observations by partnership with full joined data.
     *
     ...
     * teacher name, classroom name, and partnership name.
     *
    ...
    /**
     * Get all observations for a given mentor user across all partnerships.
     *
     ...
     *
     * @param array $data Keys: partnership_id (required), mentor_enrollment_id (required),
    ...
    public function create_observation( $data ) {
        ...
        // Verify the mentor enrollment exists and belongs to the partnership
        ...
        if ( (int) $mentor_enrollment->partnership_id !== absint( $data['partnership_id'] ) ) {
            return new WP_Error( 'partnership_mismatch', __( 'Mentor enrollment does not belong to the specified partnership.', 'hl-core' ) );
        }
    ...
    /**
     * Get the classrooms a teacher is assigned to within a specific partnership.
     *
     ...
     * the enrollment is active, with partnership name included.
     *
     * @param int $user_id WordPress user ID.
     * @return array Array of enrollment rows (ARRAY_A) with partnership_name.
     */
    public function get_mentor_enrollments( $user_id ) {
        global $wpdb;

        $rows = $wpdb->get_results( $wpdb->prepare(
            "SELECT e.*, t.partnership_name
