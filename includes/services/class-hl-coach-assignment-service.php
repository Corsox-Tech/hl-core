...
class HL_Coach_Assignment_Service {
     ...
     *
     * @param array $data Keys: coach_user_id, scope_type, scope_id, cycle_id, effective_from (optional).
     * @return int|WP_Error coach_assignment_id on success.
     */
    public function assign_coach($data) {
        global $wpdb;

        $required = array('coach_user_id', 'scope_type', 'scope_id', 'cycle_id');
    ...
    /**
     * Resolve the coach for a given enrollment in a cycle.
     *
     ...
     */
    public function get_coach_for_enrollment($enrollment_id, $cycle_id) {
        global $wpdb;
        ...
        $coach = $wpdb->get_row($wpdb->prepare(
            "SELECT ca.coach_assignment_id, ca.coach_user_id, ca.scope_type,
             ...
             LIMIT 1",
            $cycle_id, $enrollment_id, $today, $today
        ), ARRAY_A);
        ...
        $team_id = $wpdb->get_var($wpdb->prepare(
            "SELECT tm.team_id FROM {$wpdb->prefix}hl_team_membership tm
             ...
             WHERE tm.enrollment_id = %d AND t.cycle_id = %d
             LIMIT 1",
        ...
        if ($school_id) {
            $coach = $wpdb->get_row($wpdb->prepare(
                "SELECT ca.coach_assignment_id, ca.coach_user_id, ca.scope_type,
                 ...
                 ORDER BY ca.coach_assignment_id DESC
                 LIMIT 1",
                $cycle_id, $school_id, $today, $today
            ), ARRAY_A);
    ...
    /**
     * Get all current assignments for a cycle.
     *
     * @param int $cycle_id
     * @return array
     */
    public function get_assignments_by_cycle($cycle_id) {
        global $wpdb;
        ...
        return $wpdb->get_results($wpdb->prepare(
            ...
            $cycle_id, $today, $today
        ), ARRAY_A) ?: array();
    }

    /**
     * Get all assignments (including historical) for a cycle.
     *
     * @param int $cycle_id
     * @return array
     */
    public function get_all_assignments_by_cycle($cycle_id) {
        global $wpdb;
    ...
    /**
     * Get the coach's roster — all participants assigned to a coach in a cycle.
     *
     ...
     */
    public function get_coach_roster($coach_user_id, $cycle_id) {
        global $wpdb;

        $today = current_time('Y-m-d');

        // Get all active assignments for this coach in this cycle.
        $assignments = $wpdb->get_results($wpdb->prepare(
            "SELECT scope_type, scope_id FROM {$wpdb->prefix}hl_coach_assignment
             WHERE coach_user_id = %d AND cycle_id = %d
               AND effective_from <= %s
               AND (effective_to IS NULL OR effective_to >= %s)",
            $coach_user_id, $cycle_id, $today, $today
        ), ARRAY_A);
                ...
                case 'school':
                    $school_enrollments = $wpdb->get_col($wpdb->prepare(
                        "SELECT enrollment_id FROM {$wpdb->prefix}hl_enrollment
                         WHERE cycle_id = %d AND school_id = %d AND status = 'active'",
                        $cycle_id, $a['scope_id']
                    ));
     ...
     * @return array
     */
    public function get_sessions_for_enrollment($enrollment_id, $cycle_id) {
        global $wpdb;
