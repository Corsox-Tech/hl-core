...
class HL_Coach_Assignment_Service {
     ...
     *
     * @param array $data Keys: coach_user_id, scope_type, scope_id, partnership_id, effective_from (optional).
     * @return int|WP_Error coach_assignment_id on success.
     */
    public function assign_coach($data) {
        global $wpdb;

        $required = array('coach_user_id', 'scope_type', 'scope_id', 'partnership_id');
        ...
        HL_Audit_Service::log('coach_assignment.created', array(
            ...
            'entity_id'   => $id,
            'partnership_id'   => $insert['partnership_id'],
            'after_data'  => $insert,
        ));
    ...
    public function reassign_coach($old_assignment_id, $new_coach_user_id) {
        ...
        HL_Audit_Service::log('coach_assignment.closed', array(
            ...
            'partnership_id'   => $old['partnership_id'],
    ...
    public function delete_assignment($assignment_id) {
        ...
        HL_Audit_Service::log('coach_assignment.deleted', array(
            ...
            'partnership_id'   => $before['partnership_id'],
            'before_data' => $before,
        ));
    ...
    /**
     * Resolve the coach for a given enrollment in a partnership.
     *
     ...
     */
    public function get_coach_for_enrollment($enrollment_id, $partnership_id) {
        global $wpdb;
        ...
        $coach = $wpdb->get_row($wpdb->prepare(
            "SELECT ca.coach_assignment_id, ca.coach_user_id, ca.scope_type,
             ...
             LIMIT 1",
            $partnership_id, $enrollment_id, $today, $today
        ), ARRAY_A);
        ...
        $team_id = $wpdb->get_var($wpdb->prepare(
            "SELECT tm.team_id FROM {$wpdb->prefix}hl_team_membership tm
             ...
             WHERE tm.enrollment_id = %d AND t.partnership_id = %d
             LIMIT 1",
        ...
        if ($team_id) {
            $coach = $wpdb->get_row($wpdb->prepare(
                "SELECT ca.coach_assignment_id, ca.coach_user_id, ca.scope_type,
                 ...
                 ORDER BY ca.coach_assignment_id DESC
                 LIMIT 1",
                $partnership_id, $team_id, $today, $today
            ), ARRAY_A);
        ...
        if ($school_id) {
            $coach = $wpdb->get_row($wpdb->prepare(
                "SELECT ca.coach_assignment_id, ca.coach_user_id, ca.scope_type,
                 ...
                 ORDER BY ca.coach_assignment_id DESC
                 LIMIT 1",
                $partnership_id, $school_id, $today, $today
            ), ARRAY_A);
    ...
    /**
     * Get all current assignments for a partnership.
     *
     * @param int $partnership_id
     * @return array
     */
    public function get_assignments_by_partnership($partnership_id) {
        global $wpdb;
        ...
        return $wpdb->get_results($wpdb->prepare(
            ...
            $partnership_id, $today, $today
        ), ARRAY_A) ?: array();
    }

    /**
     * Get all assignments (including historical) for a partnership.
     *
     * @param int $partnership_id
     * @return array
     */
    public function get_all_assignments_by_partnership($partnership_id) {
        global $wpdb;
    ...
    /**
     * Get the coach's roster — all participants assigned to a coach in a partnership.
     *
     ...
     */
    public function get_coach_roster($coach_user_id, $partnership_id) {
        global $wpdb;

        $today = current_time('Y-m-d');

        // Get all active assignments for this coach in this partnership.
        $assignments = $wpdb->get_results($wpdb->prepare(
            "SELECT scope_type, scope_id FROM {$wpdb->prefix}hl_coach_assignment
             WHERE coach_user_id = %d AND partnership_id = %d
               AND effective_from <= %s
               AND (effective_to IS NULL OR effective_to >= %s)",
            $coach_user_id, $partnership_id, $today, $today
        ), ARRAY_A);
                ...
                case 'school':
                    $school_enrollments = $wpdb->get_col($wpdb->prepare(
                        "SELECT enrollment_id FROM {$wpdb->prefix}hl_enrollment
                         WHERE partnership_id = %d AND school_id = %d AND status = 'active'",
                        $partnership_id, $a['scope_id']
                    ));
        ...
        return $wpdb->get_results(
            "SELECT e.enrollment_id, e.partnership_id, e.roles, e.school_id,
     ...
     * @return array
     */
    public function get_sessions_for_enrollment($enrollment_id, $partnership_id) {
        global $wpdb;
        ...
        return $wpdb->get_results($wpdb->prepare(
            "SELECT cs.*, u.display_name AS coach_name
             ...
             WHERE cs.mentor_enrollment_id = %d AND cs.partnership_id = %d
