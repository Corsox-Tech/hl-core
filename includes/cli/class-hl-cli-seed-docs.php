...
HL Core is the system-of-record plugin for Housman Learning Academy. It manages everything related to your B2E (Birth to 5 Educators) programs: cycle management, participant enrollment, assessments, coaching sessions, observations, and reporting.
...
<li><strong>My Cycle</strong> — Your cycle workspace (leaders and mentors).</li>
</ul>
...
The Dashboard automatically detects your roles from your [hl_doc_link slug="enrollments-and-roles" text="enrollment records"]. A single user can have multiple roles across different cycles — the Dashboard shows the union of all relevant cards.
HTML;
    }
...
Cycles hold the configuration: linked schools, pathways, teams, enrollments, and coaching assignments.
...
An <strong>enrollment</strong> links a WordPress user to a specific Cycle. Each enrollment record contains:
<ul>
<li>The user's <strong>roles</strong> within that cycle (stored as JSON — e.g., <code>["teacher"]</code> or <code>["school_leader", "mentor"]</code>)</li>
...
<h2>Cycle roles vs WordPress roles</h2>
...
<strong>Cycle roles</strong> (teacher, mentor, school_leader, district_leader) are per-enrollment. A user can be a teacher in one cycle and a mentor in another. These roles live in the <code>hl_enrollment.roles</code> JSON column, not in WordPress user roles.

<h3>Available cycle roles</h3>
...
A <strong>School</strong> belongs to a district and represents a physical site. Schools are linked to Cycles (a Cycle can serve multiple schools), and enrollments are associated with schools.
...
A <strong>Team</strong> groups teachers together under one or two mentors for coaching and observation purposes. Teams belong to a specific Cycle and School.

<h2>Team structure</h2>

<ul>
<li>Each team has a <strong>name</strong> and is associated with a <strong>school</strong> and <strong>cycle</strong>.</li>
<li>A team can have up to <strong>2 mentors</strong> (soft limit — can be overridden).</li>
<li>A teacher can only belong to <strong>one team per cycle</strong> (hard limit).</li>
...
A <strong>control group</strong> is a Cycle where participants only complete assessments — no courses, coaching, or observations. Control groups exist for research purposes: by comparing assessment results between the program group and the control group, Housman can measure the impact of the B2E curriculum.
...
<ul>
<li><strong>Participants</strong> — Creates WordPress users, enrolls them in a cycle, assigns roles and schools. Handles identity matching (existing users are linked, not duplicated).</li>
...
<li><strong>Teaching Assignments</strong> — Links teachers (by email) to classrooms within a cycle.</li>
</ul>
...
<h3>Step 1: Upload</h3>
Select the import type, choose the target cycle, and upload a CSV file. The system validates columns and maps them to expected fields (with synonym support — e.g., "Email Address" maps to "email").
...
A <strong>teaching assignment</strong> links an enrolled teacher to a classroom within a specific cycle. When a teaching assignment is created, HL Core automatically generates [hl_doc_link slug="child-assessment" text="child assessment"] instances for that teacher-classroom pair.
...
A <strong>pathway</strong> is a structured sequence of components that participants must complete. Think of it as the "curriculum" or "program plan" for a cycle.

Each pathway has:
<ul>
<li>A <strong>name</strong> (e.g., "B2E Mastery Pathway" or "Control Group Assessment Pathway")</li>
<li>A <strong>cycle</strong> association</li>
...
You can save a pathway as a <strong>template</strong> and use it as a starting point when creating pathways for new cycles. Templates include all components, prerequisites, and drip rules, with IDs remapped to the new pathway.
...
The comparison analyzes Teacher Self-Assessment data from both cycles:
<ul>
    ...
    private function get_glossary_definitions() {
        return array(
            ...
            'glossary-enrollment' => array(
                'title'   => 'Enrollment',
                'content' => 'A record linking a WordPress user to a specific Cycle with one or more roles (teacher, mentor, school_leader, district_leader). Enrollment roles are stored per-cycle, not as WordPress roles. See [hl_doc_link slug="enrollments-and-roles"].',
            ),
            'glossary-pathway' => array(
                'title'   => 'Pathway',
                'content' => 'A structured sequence of components (the curriculum) assigned to participants within a Cycle. Pathways can be assigned by role (automatic) or explicitly (manual). See [hl_doc_link slug="pathways-overview"].',
            ),
            ...
            'glossary-control-group' => array(
                'title'   => 'Control Group',
                'content' => 'A Cycle where participants only complete assessments (no courses, coaching, or observations). Used for research purposes to measure program impact by comparison. See [hl_doc_link slug="control-groups"].',
            ),
            ...
            'glossary-teaching-assignment' => array(
                ...
                'content' => 'A record linking an enrolled teacher to a classroom within a specific Cycle. Teaching assignments trigger automatic generation of child assessment instances for that teacher-classroom pair.',
            ),
        );
    }
}
