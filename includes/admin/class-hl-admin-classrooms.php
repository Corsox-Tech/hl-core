...
class HL_Admin_Classrooms {
    ...
    private function handle_assignment_actions() {
        ...
        if (isset($_GET['remove_assignment'])) {
            ...
            wp_redirect(admin_url('admin.php?page=hl-classrooms&action=view&id=' . $classroom_id . '&cycle_id=' . $cycle_id . '&message=assignment_removed'));
            exit;
        }
    }
    ...
    private function render_teaching_assignments_section($classroom) {
        ...
        echo '<label><strong>' . esc_html__('Cycle Context:', 'hl-core') . '</strong> </label>';
        echo '<select name="cycle_id">';
        echo '<option value="">' . esc_html__('-- Select Cycle --', 'hl-core') . '</option>';
        if ($cycles) {
            foreach ($cycles as $cycle) {
                echo '<option value="' . esc_attr($cycle->cycle_id) . '"' . selected($selected_cycle, $cycle->cycle_id, false) . '>' . esc_html($cycle->cycle_name) . '</option>';
            }
        }
        echo '</select> ';
        ...
        // Current assignments table (all cycles)
        ...
        if (!empty($assignments)) {
            ...
            foreach ($assignments as $a) {
                $remove_url = wp_nonce_url(
                    admin_url('admin.php?page=hl-classrooms&action=view&id=' . $classroom->classroom_id . '&cycle_id=' . $selected_cycle . '&remove_assignment=' . $a->assignment_id),
    ...
    private function handle_child_actions() {
        ...
        if (isset($_POST['hl_child_assignment_nonce'])) {
            ...
            wp_redirect(admin_url('admin.php?page=hl-classrooms&action=view&id=' . $classroom_id . '&cycle_id=' . $cycle_id . '&message=' . $msg));
            exit;
        }
        ...
        if (isset($_GET['unassign_child'])) {
            ...
            wp_redirect(admin_url('admin.php?page=hl-classrooms&action=view&id=' . $classroom_id . '&cycle_id=' . $cycle_id . '&message=child_unassigned'));
            exit;
        }
    }
    ...
    private function render_children_roster_section($classroom) {
        ...
        if (!empty($children)) {
            ...
            foreach ($children as $child) {
                $cycle_id_param = isset($_GET['cycle_id']) ? absint($_GET['cycle_id']) : 0;
                $unassign_url = wp_nonce_url(
                    admin_url('admin.php?page=hl-classrooms&action=view&id=' . $classroom->classroom_id . '&cycle_id=' . $cycle_id_param . '&unassign_child=' . $child->child_id),
        ...
        if (!empty($unassigned) || !empty($in_other)) {
            ...
            $cycle_id_param = isset($_GET['cycle_id']) ? absint($_GET['cycle_id']) : 0;
            $form_url = admin_url('admin.php?page=hl-classrooms&action=view&id=' . $classroom->classroom_id . '&cycle_id=' . $cycle_id_param);
