...
- [x] **Phase 2B — Visual Element Picker** — `hl-element-picker.js` injected into iframe via `?hl_picker=1`. Hover highlighting, click-to-lock, 4-tier selector generation (id > unique hl-* class > ancestor path > full DOM path), postMessage with origin validation. Admin modal with "View as Role" dropdown. `is_picker_mode()` + `get_view_as_role()` on `HL_Tour_Service`. Both `HL_Tour_Service` and `HL_BuddyBoss_Integration` `get_user_hl_roles()` respect view-as override.
