...
- [~] **Seeded 14 client-requested workflows (draft state) via `bin/seed-email-workflows.php`** — Idempotent PHP seeder for rows 4, 5, 6, 7, 8, 9, 10, 12, 13, 14, 15, 16, 22, 24 of the source spreadsheet (`data/LMS Email Notification List - reorganized.xlsx` → sheet "Updated - LMS Master"). All ship `status='draft'`; admin activates per-workflow in UI after review. Uses `HL_Admin_Emails::validate_workflow_payload()` (no security bypass). Rows 2, 3 out of scope (WP user_register not a trigger). Rows 11, 17–21 awaiting Phase 2 backend wiring (stubs). Dry-run supported via `HL_SEED_DRY_RUN=1 wp eval-file`. Template/workflow names are user-agnostic — no author attribution stored. Currently deployed/run on: **test ✓, prod ✓ (2026-04-21).**
...
- [x] **Handler side-effects gating** — `HL_Frontend_Action_Plan::handle_submission()` + `HL_Frontend_RP_Notes::handle_submission()` pre-check via `get_submissions()` whether the (session, role) row is already submitted. If it is, skip `update_component_state` + `hl_core_recompute_rollups` so an edit doesn't look like a fresh completion. Handler also refactored to hoist the mentoring-only side-effect branch and harden the `$_POST` reads on RP Notes with `isset` / `wp_unslash` (matching the ticket #9 pattern on Action Plan).
- [x] **Email overdue cron fix** — `HL_Email_Automation_Service::build_completion_subquery()` for `reflective_practice_session` now joins `rpss.role_in_session = 'supervisee'` + `rps.teacher_enrollment_id = en.enrollment_id` instead of `rpss.submitted_by_user_id = en.user_id`. When a supervisor (mentor) legitimately edits the teacher's Action Plan row, `submitted_by_user_id` points at the mentor and the old subquery failed — sending a false-overdue email to the teacher. Keying on role + enrollment identifies the supervisee's submission correctly regardless of who authored the edit.
- [x] **Renderer state machine (both forms)** — `$is_submitted && !$is_edit_mode → readonly`; `$is_submitted && $is_edit_mode → editable`; `!$is_submitted → editable`. Edit mode gated on `$_GET['ap_edit'] === '1'` for Action Plan and `$_GET['rpn_edit'] === '1'` for RP Notes. Dropped the asymmetric `($context !== 'mentoring')` gate on Action Plan — now both contexts flow through the same state machine.
- [x] **Submitted status bar** — Pill ("SUBMITTED") + submit date + "edited <date>" meta (when `updated_at > submitted_at + 60s`) + [Edit Submission] button link. Replaces the old "This form has been submitted and is read-only" notice.
- [x] **Edit-mode buttons** — `[Cancel]` + `[Save Changes]` replace `[Save Draft]` + `[Submit]`. Save Changes starts disabled and enables on first dirty change; dirty hint text ("No changes yet" → "Unsaved changes" in amber) next to the button. Cancel is a plain `<a>` link that strips the edit param; confirms if dirty. Save Changes POSTs as `action=submit` (UPSERT updates the same row; submitted_at preserved per above).
- [x] **Dirty tracker + beforeunload + TinyMCE integration** — Form-serialization-based dirty check on `input`/`change`; TinyMCE-aware hook (`AddEditor` listener + 300ms/1.5s fallback init) calls `triggerSave` before serialising; `beforeunload` prompt when dirty + suppression on legitimate Save Changes or Cancel-confirmed.
- [x] **Redirect exit-edit-mode** — `handle_post_submissions()` on both RP Session + Schedule Session page controllers strip `ap_edit`/`rpn_edit` via `remove_query_arg()` before building the success redirect, so a clean Save Changes returns to the readonly view instead of staying in edit mode.
- [x] **TinyMCE enqueue for RP Notes** — Pre-existing issue: `wp_editor()` rendered plain `<textarea>` fallbacks on shortcode pages because the editor scripts never enqueued. `HL_Shortcodes::enqueue_assets()` now calls `wp_enqueue_editor()` when the post uses `[hl_component_page]` or `[hl_my_coaching]` so the bundle prints during `wp_head` (inside-the-render enqueue was too late — after `wp_head`).
