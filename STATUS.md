...
- [~] **Seeded 14 Chris workflows (draft state) via `bin/seed-chris-workflows.php`** — Idempotent PHP seeder for rows 4, 5, 6, 7, 8, 9, 10, 12, 13, 14, 15, 16, 22, 24 of the client spreadsheet. All ship `status='draft'`; Mateo activates per-workflow in admin after review. Uses `HL_Admin_Emails::validate_workflow_payload()` (no security bypass). Rows 2, 3 out of scope (WP user_register not a trigger). Rows 11, 17–21 awaiting Phase 2 backend wiring (stubs). Dry-run supported via `HL_SEED_DRY_RUN=1 wp eval-file`. Currently deployed/run on: **test pending, prod pending.**
...
- [x] **Test harness** — `bin/test-email-phase2-stubs.php` — 29 assertions across all 3 sections, DB fixtures under `[Phase2Test]` prefix with finally-block cleanup. Handler SQL tested via reflection; NULL-enrollment coach path tested end-to-end through `run_daily_checks()`. All green on test (SHA `2b7c692`, 2026-04-21).
- [~] **Version bump 1.2.8 → 1.2.9** — bumps Phase 2 boundary. HL_CORE_VERSION updated.
