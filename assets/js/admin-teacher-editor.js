    ...
    function initTitleSyncInContainer(container) {
        container.querySelectorAll('.hl-te-section-title-input').forEach(function (input) {
            ...
            input.addEventListener('input', function () {
                ...
                // Auto-populate key for new sections (has hl-te-auto-key class)
                var keyInput = panel.querySelector('.hl-te-auto-key');
                if (keyInput) {
                    keyInput.value = slugify(input.value);
                }
            });
        });
    }
    ...
    function initRemoveHandlers(container) {
        ...
        container.querySelectorAll('.hl-te-add-context-opt').forEach(function (btn) {
            ...
            btn.addEventListener('click', function () {
                ...
                tr.innerHTML =
                    '<td><em style="color:#888;">' + escHtml(i18n.autoGenerated || 'Auto-generated') + '</em>' +
                        '<input type="hidden" name="sections[' + secIdx + '][context_keys][]" value="" /></td>' +
            ...
            panel.innerHTML =
                ...
                '<div class="hl-te-section-body">' +
                    '<table class="form-table">' +
                        ...
                        '<td><input type="text" name="' + prefix + '[section_key]" value="" class="regular-text hl-te-auto-key" placeholder="' + escHtml(i18n.autoGenerated || 'Auto-generated from title') + '" />' +
                        '</td></tr>' +
